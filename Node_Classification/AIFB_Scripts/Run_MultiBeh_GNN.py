...
sys.path.append("/Users/niharika.dsouza/Projects/refactored/")
...
from utils.graph_object_creator import heterograph_creater,create_CIKM_prop_graph
from utils.evaluators import performance_evaluate_multiplex
from utils.neural_network_modules import Multiplex_CIKM
...
if __name__ == "__main__":
    ...
    # CIKM Multiplex
    print("Training MultiBehav GNN Model")
    G_CIKM = create_CIKM_prop_graph(G)
    CIKM_model = Multiplex_CIKM(G_CIKM[0],G_CIKM[1],8, 8, n_classes, num_rels)
    train_multiplex(G_CIKM[0],G_CIKM[1], CIKM_model,data_splits, labels, num_levels = num_rels, lr = 0.001, num_epochs = 40) #60
    CIKM_mplex_ROC = performance_evaluate_multiplex(CIKM_model,G_CIKM,labels,test_idx,num_rels,'Multi Behav. GNN ',folder_name)
    
    for class_no in range(len(CIKM_mplex_ROC)):
            mlflow.log_metric("MultiBehGNN_AUROC_class " + str(class_no), CIKM_mplex_ROC[class_no])
    
    f = open(folder_name + '/MultiBehGNN_model_'+ str(run_no) +'.p','wb') 
    pickle.dump(CIKM_model.state_dict(),f)
    f.close()